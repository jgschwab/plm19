...
This changes each item in the list of "has" things so it is just the first atom.
For example, this changes "(balance .05)" to just "balance". This is because we don't need the default values to build the "getter" methods. Those are used when the klass is made.


TODO 1b. Why is message set to a gensym?

gensym renames the message to a unique identifier.
This is to avoid conflicts with the code inside the macro.
...
|#
(defun datas-as-case (properties)
  (mapcar (lambda (propName)
      `(,propName (lambda nil ,propName)))
     properties))
